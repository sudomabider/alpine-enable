...
func TestExpand(t *testing.T) {
	c, err := parseArgs([]string{"git", "postgres", "mysql", "zip", "soap", "crux", "swoole", "pm2", "mcrypt", "gd", "imap", "xmlrpc"}, recipe)
	...
	expected := command{
		system:      []string{"git", "imap-dev", "libmcrypt-dev", "libpng-dev", "libxml2-dev", "libzip-dev", "mysql-client", "nodejs", "npm", "openssh-client", "postgresql-dev", "zlib-dev"},
		build:       []string{"$PHPIZE_DEPS"},
		phpInstall:  []string{"bcmath", "gd", "imap", "mcrypt", "mysql", "pcntl", "pdo_mysql", "pdo_pgsql", "pgsql", "soap", "sockets", "xmlrpc", "zip"},
